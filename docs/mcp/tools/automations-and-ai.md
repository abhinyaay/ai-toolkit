...
| `get_automations` | Yes | Lists rules with `event_id`, `event_params`, and `condition`; optional `organization_id` and/or `pipe_id`. Use `get_automation` for full action parameters. |
