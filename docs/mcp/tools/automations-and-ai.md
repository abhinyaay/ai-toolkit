...
**Event and action compatibility.** A pair is invalid when the `event_id` appears in that action's `eventsBlacklist` (`get_automation_actions`), equivalently when the `action_id` appears in that event's `actionsBlacklist` (`get_automation_events`); the two lists agree throughout the catalog. `triggerEvents` is not that list. It reflects which pairings the builder offers first, and pairs outside it execute normally, so a membership test on `triggerEvents` refuses valid rules. The API rejects a blacklisted pair on both `create_automation` and `update_automation` with an untranslated error whose readable part is the key `event_action_blacklist`, and writes nothing.
