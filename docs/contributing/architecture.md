...
- Agent. Qualified at each use, because this document carries three senses and no default. An AI agent is a Pipefy entity that a builder defines in a pipe, and it acts inside a process as a non-human assignee. An LLM agent is a program that runs a model's decisions and reaches the toolkit from outside, which the `Stakeholders` table names. A contributing agent opens a pull request, under [`AGENTS.md`](../../AGENTS.md).
